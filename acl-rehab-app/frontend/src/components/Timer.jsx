...
export default function Timer({ defaultText }) {
  ...
  return (
    <div className="rounded-xl border border-slate-300 bg-slate-50 p-3">
      <p className="text-4xl font-bold text-center">{mm}:{ss}</p>
      <div className="mt-3 grid grid-cols-3 gap-2">
        <button className="touch-btn bg-accent text-white" onClick={() => setRunning((v) => !v)}>{running ? "Pauze" : "Start"}</button>
        <button className="touch-btn bg-slate-300" onClick={() => { setSecondsLeft(initial); setRunning(false); }}>Reset</button>
