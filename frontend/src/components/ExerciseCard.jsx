    ...
    <div className="card border-l-[3px] border-l-[#007aff]">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[13px] text-[#8e8e93]">{exercise.default_sets} sets · {exercise.default_reps}</p>
          {showWarning && <p className="mt-1 text-xs font-semibold text-ios-red">Max 70-80° knieflexie i.v.m. meniscus.</p>}
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <input className="ios-input" value={state.reps_completed} onChange={(e) => onChange("reps_completed", e.target.value)} placeholder="Reps" />
        <input className="ios-input" value={state.weight_used} onChange={(e) => onChange("weight_used", e.target.value)} placeholder="Gewicht" />
      </div>
